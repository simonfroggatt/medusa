...
from apps.category.models import OcTsgCategoryStoreParent, OcCategory, OcCategoryDescription, OcCategoryDescriptionBase
...
class GoogleMerchantViewSet(viewsets.ViewSet):
    ...
    def _add_variant_to_channel(self, channel, product_data, variant):
        ...
        description.text = product_data.get('description')
        
        link = ET.SubElement(item, 'link')
        link.text = variant.get('link')
        ...
        price.text = f"{variant.get('price')} GBP"
    ...
    def _get_product_category(self, product):
        ...
        if not product_category:
            return ''
        category_path = self._get_category_parent(product_category, '')
        return category_path


    def _get_category_parent(self, category, current_path):
            ...
            for core_variant in core_variants:
                ...
                variant_data = {
                    'id': f'{core_variant.prod_variant_core_id}v20',
                     #'id': f'{store.store_id}-{product.product_id}-{core_variant.prod_variant_core_id}',
                    ...
                    'link': f"{product_link}&variantid={core_variant.prod_variant_core_id}",
                    ...
                    'custom_label_1': size_name_adwords,
    ...
    def _get_size_name(self, product_size):
        ...
        if product_size.orientation_id == 4:
            ...
            size_name = f'{product_size.size_width} x {product_size.size_height}mm'
        return size_name
