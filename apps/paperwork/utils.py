...
def _create_image_url(image_src):
    ...
    image_safe_path = urlunparse(parsed._replace(path=safe_path))

    if image_src.endswith('.svg'):
        if settings.CDN:
            svg_url = image_safe_path
        else:
